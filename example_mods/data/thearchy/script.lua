...
function genWinName()
  ...
  realname = "GODS RULE"
  for i = 1,15 do
    if math.random(1,90) > 60 then
      name = name .. tostring('@')
    else
      name = name .. tostring(string.sub(realname,i,i))
    end
  end
  return name;
end
...
function onUpdate(elapsed)
  ...
  setPropertyFromClass("openfl.Lib", "application.window.title", genWinName())
  ...
  if curStep >= 0 then
   ...
   if increaseChartY == true then
    modchartY = modchartY + 0.5
  end

  if increaseChartX == true then
    modchartX = modchartX + 1
  end
  ...
  --windowZooming();
   ...
   if spin == true then
        for i = 0,7 do 
            setPropertyFromGroup('strumLineNotes', i, 'x', defaultNotePos[i + 1][1] + modchartX * math.sin((currentBeat + i*0.50) * math.pi))
...
function onDestroy()
        ...
        function onBeatHit()
          ...
          if curBeat < 50000 then
            ...
            doTweenAngle('turngame', 'camGame', thing * 4, crochet / 500, 'quadInOut')
